...
class StaffRoleSeeder extends Seeder
{
    ...
    private function ensureManagePermissions(array $permissions): array
    {
        ...
        // If a role can access products, it should also be able to access GRNs
        if (collect($permissions)->contains(function ($permission) {
            return str_ends_with($permission, '-products');
        })) {
            if (! in_array('view-grns', $processedPermissions, true)) {
                $managePermissions[] = 'view-grns';
            }
            if (! in_array('manage-grns', $processedPermissions, true)) {
                $managePermissions[] = 'manage-grns';
            }
        }
    ...
    public function run(): void
    {
        ...
        if (IsDemo()) {
            $roleTemplates = [
                ...
                [
                    'name' => 'salesrep',
                    'label' => 'Sales Rep',
                    'description' => 'Sales Rep has access to sales operations with delivery routes',
                    'permissions' => [
                        'manage-dashboard',
        ...
        foreach ($companyUsers as $company) {
            ...
            foreach ($roleTemplates as $roleTemplate) {
                ...
                // Get permissions for this role (including derived manage permissions)
                $permissions = $this->ensureManagePermissions($roleTemplate['permissions']);
