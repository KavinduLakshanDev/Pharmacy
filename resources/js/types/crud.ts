...
export interface TableColumn {
  ...
  export interface TableAction {
    ...
    /**
     * If true, the action button will be rendered but disabled.
     * Can be a boolean or a function that receives the row and returns a boolean.
     */
    disabled?: boolean | ((row: any) => boolean);
    /**
     * Optional tooltip to show when the action is disabled.
     */
  ...
  export interface FormField {
    ...
    inline?: boolean; // For multi-select: show list always visible instead of dropdown
    options?: FilterOption[];
    ...
    onWheel?: (event: React.WheelEvent<HTMLInputElement>) => void; // For number inputs to prevent scroll change
