...
export default function GrnEdit() {
  ...
  const { grn, suppliers = [], branches = [], products = [], statuses = [], discountTypes = [] } = usePage().props as any;
  ...
  const [formData, setFormData] = useState({
    ...
    grn_date: grn.grn_date?.split('T')[0] || new Date().toISOString().split('T')[0],
    description: grn.description || '',
    status: grn.status || (statuses?.includes('pending') ? 'pending' : statuses?.[0] ?? ''),
  });
  ...
  const handleEditItem = (index: number) => {
    ...
    setIsManualSalePrice(true); // Treat existing items as manually set so we don't overwrite them on load
    setItemModalStep('details');
    setIsItemModalOpen(true);
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    toast.loading(t('Updating GRN...'));

    router.put(route('grns.update', grn.id), payload, {
      onSuccess: (page: any) => {
        toast.dismiss();
        if (page.props?.flash?.success) {
          toast.success(t(page.props.flash.success));
        } else {
          toast.success(t('GRN updated successfully'));
        }
      },
      onError: (errors: any) => {
        toast.dismiss();
        const errorMessage = typeof errors === 'object' ? Object.values(errors).flat().join(', ') : errors;
        toast.error(t('Failed to update: {{errors}}', { errors: errorMessage }) || t('Failed to update GRN'));
      }
    });
  };
  ...
  return (
    <PageTemplate
      title={t('Edit GRN')}
      description={t('Update existing Goods Received Note')}
      breadcrumbs={breadcrumbs}
      url={`/grns/${grn.id}/edit`}
      actions={[
        {
    ...
    >
      <form onSubmit={handleSubmit} className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex justify-end gap-2">
          ...
          <Button type="submit">{t('Update GRN')}</Button>
        </div>
      </form>
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {itemModalStep === 'select'
                ...
                : t('Review and edit details before saving.')}
            </DialogDescription>
          </DialogHeader>
          ...
          {itemModalStep === 'select' ? (
            ...
            <div className="space-y-4">
              ...
              <DialogFooter className="justify-end">
                ...
                <Button type="button" onClick={handleSaveItem}>{t('Save Item')}</Button>
              </DialogFooter>
            </div>
          )}
        </DialogContent>
