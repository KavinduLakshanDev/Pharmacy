...
import { Calendar, MapPin, CreditCard, FileText, Download, Printer, Package, DollarSign, User, Copy, Check } from 'lucide-react';
...
const PRINT_STYLES = {
  ...
  colorAdjust: '-webkit-print-color-adjust: exact !important; color-adjust: exact !important; print-color-adjust: exact !important;',
  body: 'margin: 0; padding: 0; font-size: 11pt; line-height: 1.3; color: #000 !important; background: white !important;',
  container: 'max-width: 100% !important; margin: 0 !important; padding: 10px !important; font-size: 12px !important;',
  hide: 'display: none !important;',
  card: 'border: 1px solid #e5e7eb !important; border-radius: 8px !important; margin-bottom: 15px !important; break-inside: avoid;',
  table: 'width: 100% !important; border-collapse: collapse !important; margin: 0 !important;',
  tableCell: 'border: 1px solid #d1d5db !important; padding: 8px 6px !important; text-align: left; font-size: 10pt;',
  grid: 'display: grid !important; grid-template-columns: repeat(3, 1fr) !important; gap: 8px !important; margin-bottom: 20px !important;'
};

const printStyles = `@media print {
  @page { ${PRINT_STYLES.page} }
  * { ${PRINT_STYLES.colorAdjust} }
  body { ${PRINT_STYLES.body} }
  .print-container { ${PRINT_STYLES.container} }
  .print-hide { ${PRINT_STYLES.hide} }
  .print-card { ${PRINT_STYLES.card} }
  .print-table { ${PRINT_STYLES.table} }
  .print-table th, .print-table td { ${PRINT_STYLES.tableCell} }
  .print-table th { background: #f3f4f6 !important; font-weight: bold; }
  .print-summary-cards { ${PRINT_STYLES.grid} }
  .print-billing-grid { display: grid !important; grid-template-columns: 1fr 1fr !important; gap: 20px !important; }
}`;
...
export default function PublicInvoice({ invoice, templateId, company }: Props) {
    ...
    const themeColors = { blue: '#3b82f6', green: '#10b981', purple: '#8b5cf6', orange: '#f97316', red: '#ef4444' };
    const brandThemeColor = themeColor === 'custom' ? customColor : themeColors[themeColor as keyof typeof themeColors] || '#3b82f6';

    // Use template colors if templateId is provided, otherwise use brand theme
    const template = templateId ? templates[templateId as keyof typeof templates] || templates[1] : { primary: brandThemeColor, secondary: brandThemeColor };
    const currentThemeColor = template.primary;
    ...
    // Apply theme color to CSS custom property for print styles
    React.useEffect(() => {
    ...
    return (
        <>
            ...
            <div className="min-h-screen bg-gray-50 dark:from-gray-900 dark:to-gray-800 py-8">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 print-container">
                    {/* Action Bar */}
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 print-hide">
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-white rounded-lg shadow-sm border">
                            ...
                            <div className="p-8">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 print-billing-grid">
                                    <div>
