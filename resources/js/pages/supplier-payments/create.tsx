...
import { AlertCircle, ArrowLeft, Building2, CreditCard, DollarSign, Receipt, Search } from 'lucide-react';
...
export default function SupplierPaymentCreate() {
    ...
    return (
        <PageTemplate
            title={t('Record Supplier Payment')}
            description={t('Record a new payment to a supplier')}
            url="/inventory/supplier-payments/create"
            breadcrumbs={breadcrumbs}
            actions={[
                {
                    label: t('Back to Supplier Payments'),
                    ...
                    <div className="lg:col-span-1">
                        <Card>
                            ...
                            <CardContent className="space-y-4">
                                <div>
                                    <Label htmlFor="supplier-search">{t('Search Supplier')}</Label>
                                    <Input
                                        id="supplier-search"
                                        type="text"
                                ...
                                {!selectedSupplier && suppliers.length === 0 && (
                                    <p className="rounded-lg border border-dashed p-3 text-center text-sm text-muted-foreground">
                                        {t('Search and select a supplier to view their outstanding balance.')}
                                    </p>
                                )}
                                ...
                                {selectedSupplier && (
                                    <div className="rounded-lg border bg-muted/40 p-4">
                                        ...
                                        {selectedSupplier.LstNm && (
                                            <p className="mt-1 text-sm text-muted-foreground">{t('Contact')}: {selectedSupplier.LstNm}</p>
                                        )}
                                        <p className="text-sm text-muted-foreground">{t('Phone')}: {selectedSupplier.TP1 || '-'}</p>
                                        <div className="mt-3 flex items-center justify-between rounded-md bg-background p-2">
                                            <span className="text-sm font-medium">{t('Outstanding Balance')}</span>
                        ...
                        <Card>
                            <CardHeader>
                                <CardTitle className="flex items-center gap-2">
                                    ...
                                    {selectedSupplier ? `${t('Record Payment for')} ${selectedSupplier.FstNm}` : t('Record Payment')}
                                </CardTitle>
                            </CardHeader>
                            <CardContent>
                                {!selectedSupplier && (
                                    <div className="mb-6 rounded-lg border border-amber-200 bg-amber-50 p-4 dark:border-amber-800 dark:bg-amber-950/30">
                                        <div className="flex items-start gap-3">
                                            ...
                                            <div>
                                                <h4 className="font-semibold text-amber-900 dark:text-amber-200">{t('Supplier Required')}</h4>
                                                <p className="mt-1 text-sm text-amber-700 dark:text-amber-300">{t('Please select a supplier from the left panel before recording a payment.')}</p>
                                            </div>
                                        </div>
                                    </div>
                                )}
