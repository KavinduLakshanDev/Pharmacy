...
export default function StockTransferEdit() {
  ...
  const { transfer, branches = [], products = [], branchBatchStock = {}, statuses = [] } = usePage().props as any;

  const [formData, setFormData] = useState({
    transfer_no: transfer.transfer_no ?? '',
    from_branch_id: String(transfer.from_branch_id ?? ''),
    to_branch_id: String(transfer.to_branch_id ?? ''),
    transfer_date: transfer.transfer_date ? transfer.transfer_date.split('T')[0] : new Date().toISOString().split('T')[0],
  ...
  const [items, setItems] = useState<TransferItem[]>(
    (transfer.items ?? []).map((item: any) => ({
      ...
      unit_cost_price: item.unit_cost_price != null ? Number(item.unit_cost_price) : null,
    })),
  );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    toast.loading(t('Updating stock transfer...'));

    router.put(route('inventory.stock-transfers.update', transfer.id), { ...formData, items }, {
      onSuccess: (page: any) => {
        toast.dismiss();
        if (page.props?.flash?.success) {
          toast.success(t(page.props.flash.success));
        } else {
          toast.success(t('Stock transfer updated successfully'));
        }
      },
      onError: (errors: any) => {
        toast.dismiss();
        const message = typeof errors === 'object' ? Object.values(errors).flat().join(', ') : errors;
        toast.error(message || t('Failed to update stock transfer'));
      },
    });
  };
  ...
  return (
    <PageTemplate
      title={t('Edit Stock Transfer')}
      description={t('Update stock transfer details')}
      breadcrumbs={breadcrumbs}
      url={`/inventory/stock-transfers/${transfer.id}/edit`}
      actions={[
        {
          label: t('Back'),
          variant: 'outline',
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <Select value={formData.from_branch_id} onValueChange={(value) => handleFormChange('from_branch_id', value)}>
                  <SelectTrigger>
              ...
              <div>
                ...
                <Select value={formData.to_branch_id} onValueChange={(value) => handleFormChange('to_branch_id', value)}>
                  <SelectTrigger>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center justify-between">
              <p className="text-sm text-gray-500">{t('Update the products and quantities to transfer.')}</p>
