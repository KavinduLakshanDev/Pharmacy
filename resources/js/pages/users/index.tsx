...
export default function Users() {
    ...
    if (formMode === 'create') {
      ...
      router.post(route('users.store'), formData, {
        onSuccess: (page) => {
          ...
          toast.dismiss();
          const flash = page.props.flash as { success?: string; error?: string; warning?: string };
          if (flash.success) {
  ...
  if (hasPermission(permissions, 'create-users')) {
    ...
    pageActions.push({
      ...
      variant: (canCreate ? 'default' : 'outline') as 'default' | 'outline',
    ...
    >
      ...
      <CrudFormModal
        ...
        formConfig={{
          fields: [
            ...
            ...(branches && branches.length > 0 ? [{
              ...
              inline: true,
              placeholder: t('Select branches (optional)'),
        ...
        initialData={currentItem ? {
          ...
          branch_ids: currentItem.branches ? currentItem.branches.map((b: any) => b.id.toString()) : []
        } : null}
        title={
          formMode === 'create'
            ? t('Add New User')
