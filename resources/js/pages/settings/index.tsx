...
export default function Settings() {
  ...
  const sidebarNavItems = allSidebarNavItems.filter(item => {
    ...
    if (auth.user && auth.user.type === 'company') {
      ...
      return ['manage-system-settings', 'manage-email-settings', 'manage-brand-settings', 'manage-currency-settings', 'manage-webhook-settings', 'manage-email-notifications', 'manage-twilio-notifications', 'settings'].includes(item.permission);
    }
    return false;
  });
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const companySettingsPosition = companySettingsRef.current?.offsetTop || 0;
      ...
      } else if (scrollPosition >= companySettingsPosition) {
  ...
  const renderSidebarIcon = (icon: NavItem['icon']) => {
      ...
      {/* <div className="flex flex-col md:flex-row gap-8"> */}
        ...
        <div className="flex-1">
          ...
          {auth.user?.type === 'company' && (
            <section id="company-settings" ref={companySettingsRef} className="mb-8">
              <CompanySettings />
            </section>
          )}
