...
export default function SystemSettings({
  ...
  const [systemSettings, setSystemSettings] = useState(() => ({
    ...
    enable_privileged_points: settingsData.enable_privileged_points === 'true' || settingsData.enable_privileged_points === true || settingsData.enable_privileged_points === '1' || (settingsData.enable_privileged_points === undefined ? defaultSettings.enable_privileged_points : false)
  }));
  ...
  useEffect(() => {
    if (Object.keys(settingsData).length > 0) {
      ...
      Object.keys(defaultSettings).forEach((key) => {
        const val = settingsData[key] !== undefined ? settingsData[key] : (defaultSettings as any)[key];
      ...
      setSystemSettings(prevSettings => ({
        ...prevSettings,
        ...mergedSettings,
        emailVerification: String(mergedSettings.emailVerification) === 'true' || mergedSettings.emailVerification === true || String(mergedSettings.emailVerification) === '1',
        landingPageEnabled: String(mergedSettings.landingPageEnabled) === 'true' || mergedSettings.landingPageEnabled === true || String(mergedSettings.landingPageEnabled) === '1' || (mergedSettings.landingPageEnabled === undefined ? defaultSettings.landingPageEnabled : false),
        enable_privileged_points: String(mergedSettings.enable_privileged_points) === 'true' || mergedSettings.enable_privileged_points === true || String(mergedSettings.enable_privileged_points) === '1' || (mergedSettings.enable_privileged_points === undefined ? defaultSettings.enable_privileged_points : false)
      }));
    }
  }, [settingsData]);
  ...
  const submitSystemSettings = (e: React.FormEvent) => {
    ...
    const cleanSettings = {
      ...
      enable_privileged_points: Boolean(systemSettings.enable_privileged_points)
    };
      ...
      <form id="system-settings-form" onSubmit={submitSystemSettings} className="space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="grid gap-2 md:col-span-2">
            <div className="flex items-center justify-between">
              <div className="space-y-0.5">
                <Label htmlFor="enable_privileged_points">{t("Privileged Customer Points")}</Label>
                <p className="text-sm text-muted-foreground">
                  {t("Enable loyalty points calculation and balance updates for privileged customers upon sale completion")}
                </p>
              </div>
              <Switch
                id="enable_privileged_points"
                checked={systemSettings.enable_privileged_points}
                onCheckedChange={(checked) => handleSystemSettingsChange('enable_privileged_points', checked)}
              />
            </div>
          </div>
        </div>
