...
import { AlertCircle, ArrowLeft, CreditCard, DollarSign, Receipt, Search } from 'lucide-react';
...
export default function CustomerPaymentCreate() {
    ...
    const breadcrumbs = [
        ...
        { title: t('Customer Payments'), href: route('inventory.customer-payments.index') },
        { title: t('Create Payment') },
    ];

    const paymentMethods = [
        { value: 'Cash', label: t('Cash'), icon: DollarSign },
        { value: 'Cheque', label: t('Cheque'), icon: Receipt },
        { value: 'Bank', label: t('Bank Deposit'), icon: CreditCard },
        { value: 'Online Transfer', label: t('Online Transfer'), icon: CreditCard },
    ];

    return (
        <PageTemplate
            title={t('Record Customer Payment')}
            description={t('Record a new payment from a customer')}
            url="/inventory/customer-payments/create"
            breadcrumbs={breadcrumbs}
            actions={[
                {
                    label: t('Back to Customer Payments'),
                    icon: <ArrowLeft className="h-4 w-4 mr-2" />,
                    variant: 'outline',
                    onClick: () => router.visit(route('inventory.customer-payments.index')),
                },
            ]}
        >
                    ...
                    <div className="lg:col-span-1">
                        <Card>
                            ...
                            <CardContent className="space-y-4">
                                <div>
                                    <Label htmlFor="customer-search">{t('Search Customer')}</Label>
                                    <Input
                                        id="customer-search"
                                        type="text"
                                        placeholder={t('Search by name, code, phone, or email...')}
                                        value={searchTerm}
                                        ...
                                        {customers.map((customer) => (
                                            ...
                                            >
                                                ...
                                                <div className="mt-1 flex items-center justify-between text-sm">
                                                    <span className="font-medium">{t('Current Balance')}</span>
                                ...
                                {!selectedCustomer && customers.length === 0 && (
                                    <p className="rounded-lg border border-dashed p-3 text-center text-sm text-muted-foreground">
                                        {t('Search and select a customer to view their current balance.')}
                                    </p>
                                )}
                                ...
                                {selectedCustomer && (
                                    <div className="rounded-lg border bg-muted/40 p-4">
                                        ...
                                        <p className="text-sm text-muted-foreground">{t('Code')}: {selectedCustomer.AdrCd || '-'}</p>
                                        <p className="text-sm text-muted-foreground">{t('Phone')}: {selectedCustomer.TP1 || '-'}</p>
                                        <div className="mt-3 flex items-center justify-between rounded-md bg-background p-2">
                                            <span className="text-sm font-medium">{t('Current Balance')}</span>
                                ...
                                {recentPayments.length > 0 && (
                                    <div className="space-y-2">
                                        <h4 className="text-sm font-semibold">{t('Recent Payments')}</h4>
                                ...
                                {outstandingInvoices.length > 0 && (
                                    <div className="space-y-2">
                                        <h4 className="text-sm font-semibold">{t('Outstanding Invoices')}</h4>
                                            ...
                                            <table className="w-full text-xs">
                                                <thead>
                                                    <tr className="border-b bg-muted/50">
                                                        ...
                                                        <th className="px-2 py-2 text-right font-medium">{t('Pay Amount')}</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                        ...
                        <Card>
                            <CardHeader>
                                <CardTitle className="flex items-center gap-2">
                                    ...
                                    {selectedCustomer ? `${t('Record Payment for')} ${selectedCustomer.FstNm}` : t('Record Payment')}
                                </CardTitle>
                            </CardHeader>
                            <CardContent>
                                {!selectedCustomer && (
                                    <div className="mb-6 rounded-lg border border-amber-200 bg-amber-50 p-4 dark:border-amber-800 dark:bg-amber-950/30">
                                        <div className="flex items-start gap-3">
                                            ...
                                            <div>
                                                <h4 className="font-semibold text-amber-900 dark:text-amber-200">{t('Customer Required')}</h4>
                                                <p className="mt-1 text-sm text-amber-700 dark:text-amber-300">{t('Please select a customer from the left panel before recording a payment.')}</p>
                                            </div>
                                        </div>
                                    </div>
                                )}
                                ...
                                <form onSubmit={handleSubmit} className="space-y-6">
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div>
                                            <Label htmlFor="paid_amount">{t('Amount (Rs.)')}</Label>
                                            <Input
                                                id="paid_amount"
                                                type="number"
                                                step="0.01"
                                    ...
                                    {(data.payment_method === 'Cheque' || data.payment_method === 'Bank' || data.payment_method === 'Online Transfer') && (
                                        <div>
                                            <Label>{t('Select Bank Account')}</Label>
                                            <Select onValueChange={handleBankSelection}>
                                                <SelectTrigger className="mt-1">
                                                    <SelectValue placeholder={t('Choose a bank account')} />
                                                </SelectTrigger>
                                                <SelectContent>
                                    ...
                                    {data.payment_method === 'Cheque' && (
                                        <div className="space-y-4 rounded-lg border p-4">
                                            <h3 className="font-semibold">{t('Cheque Details')}</h3>
                                            <div className="grid grid-cols-2 gap-4">
                                                <Input placeholder={t('Cheque Number')} value={data.cheque_no} onChange={(event) => setData('cheque_no', event.target.value)} />
                                                <Input type="date" value={data.cheque_date} onChange={(event) => setData('cheque_date', event.target.value)} />
                                                <Input placeholder={t('Bank Name')} value={selectedBank ? selectedBank.name : data.cheque_bank_name} onChange={(event) => setData('cheque_bank_name', event.target.value)} readOnly={!!selectedBank} />
                                                <Input placeholder={t('Account Number')} value={selectedBank ? (selectedBank.bank_account_no ?? '') : data.cheque_account_no} onChange={(event) => setData('cheque_account_no', event.target.value)} readOnly={!!selectedBank} />
                                                <Input placeholder={t('Branch')} value={selectedBank ? (selectedBank.bank_branch ?? '') : data.cheque_branch} onChange={(event) => setData('cheque_branch', event.target.value)} readOnly={!!selectedBank} />
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    {data.payment_method === 'Bank' && (
                                        <div className="space-y-4 rounded-lg border p-4">
                                            <h3 className="font-semibold">{t('Bank Deposit Details')}</h3>
                                            <div className="grid grid-cols-2 gap-4">
                                                <Input placeholder={t('Bank Name')} value={selectedBank ? selectedBank.name : data.bank_name} onChange={(event) => setData('bank_name', event.target.value)} readOnly={!!selectedBank} />
                                                <Input placeholder={t('Reference No')} value={data.bank_reference_no} onChange={(event) => setData('bank_reference_no', event.target.value)} />
                                                <Input type="date" value={data.bank_deposit_date} onChange={(event) => setData('bank_deposit_date', event.target.value)} />
                                                <Input placeholder={t('Branch')} value={selectedBank ? (selectedBank.bank_branch ?? '') : data.bank_branch} onChange={(event) => setData('bank_branch', event.target.value)} readOnly={!!selectedBank} />
                                                <Input placeholder={t('Account Number')} value={selectedBank ? (selectedBank.bank_account_no ?? '') : data.bank_account_no} onChange={(event) => setData('bank_account_no', event.target.value)} readOnly={!!selectedBank} />
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    {data.payment_method === 'Online Transfer' && (
                                        <div className="space-y-4 rounded-lg border p-4">
                                            <h3 className="font-semibold">{t('Online Transfer Details')}</h3>
                                            <div className="grid grid-cols-2 gap-4">
                                                <Input placeholder={t('Reference Number')} value={data.transfer_reference_no} onChange={(event) => setData('transfer_reference_no', event.target.value)} />
                                                <Input placeholder={t('Transaction ID')} value={data.transfer_transaction_id} onChange={(event) => setData('transfer_transaction_id', event.target.value)} />
                                                <Input type="date" value={data.transfer_date} onChange={(event) => setData('transfer_date', event.target.value)} />
                                                <Input placeholder={t('Bank Name')} value={selectedBank ? selectedBank.name : data.transfer_bank_name} onChange={(event) => setData('transfer_bank_name', event.target.value)} readOnly={!!selectedBank} />
                                                <Input placeholder={t('Branch')} value={selectedBank ? (selectedBank.bank_branch ?? '') : data.transfer_branch} onChange={(event) => setData('transfer_branch', event.target.value)} readOnly={!!selectedBank} />
                                            </div>
                                        </div>
                                    )}

                                    <div>
                                        <Label htmlFor="notes">{t('Description (Optional)')}</Label>
                                        <Textarea
                                            id="notes"
                                            value={data.notes}
