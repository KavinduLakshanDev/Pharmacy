...
export default function CustomerPaymentsIndex() {
    ...
    const { auth, payments = [], filters: pageFilters = {} } = usePage().props as any;
    ...
    const applyFilters = () => {
        router.get(route('inventory.customer-payments.index'), {
            page: 1,
    ...
    const handleAction = (action: string, item: any) => {
        if (action === 'view') {
            router.visit(route('inventory.customer-payments.receipt', item.id));
        }
    };
    ...
    const columns = [
        {
            ...
            render: (value: any) => (
                <div className="flex items-center gap-2">
                    ...
                    <span className="font-mono text-sm">CP-{value}</span>
                </div>
            )
        },
        {
    ...
    const pageActions = [
        {
            ...
            onClick: () => router.visit(route('inventory.customer-payments.create')),
        },
    ];
    ...
    return (
        <PageTemplate
            title={t('Customer Payments')}
            description={t('Manage and record customer payments')}
