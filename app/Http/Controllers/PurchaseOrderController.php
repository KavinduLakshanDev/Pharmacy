...
use App\Models\Tax;
use App\Services\PurchaseOrderSupplierRecommendationService;
use Illuminate\Http\Request;
use Inertia\Inertia;

class PurchaseOrderController extends Controller
{
    public function __construct(private PurchaseOrderSupplierRecommendationService $supplierRecommendationService) {}

    public function index(Request $request)
    {
        $query = PurchaseOrder::query()
            ->with(['salesOrder', 'supplier', 'account', 'billingContact', 'shippingContact', 'shippingProviderType', 'creator', 'assignedUser', 'products.tax'])
            ->where(function ($q) {
        ...
        if ($request->has('supplier_id') && ! empty($request->supplier_id) && $request->supplier_id !== 'all') {
        ...
        return Inertia::render('purchase-orders/index', [
            ...
            'users' => $users,
            'filters' => $request->all(['search', 'status', 'supplier_id', 'account_id', 'assigned_to', 'sort_field', 'sort_direction', 'per_page']),
        ]);
    }
    ...
    public function recommendSuppliers(Request $request)
    {
        ...
        $recommendations = $this->supplierRecommendationService->recommend(
    ...
    public function destroy($purchaseOrderId)
    {
        ...
        return redirect()->route('purchase-orders.index')->with('success', __('Purchase order deleted successfully.'));
    }
    ...
    public function toggleStatus($purchaseOrderId)
    {
        ...
        return redirect()->route('purchase-orders.index')->with('success', __('Purchase order status updated successfully.'));
    }
