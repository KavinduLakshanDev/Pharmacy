...
class EmailSettingController extends Controller
{
    ...
    public function sendTestEmail(Request $request)
    {
        ...
        $password = getSetting('email_password', '');
        $mailScheme = $settings['encryption'] === 'ssl' ? 'smtps' : 'smtp';

        if (
            empty($settings['host']) || $settings['host'] === 'smtp.example.com' ||
            empty($settings['username']) || $settings['username'] === 'user@example.com' ||
            empty($password)
        ) {
            return redirect()->back()->with('error', __('Please configure valid email host, username, and password before sending a test email.'));
        }
        ...
        try {
            ...
            config([
                ...
                'mail.mailers.smtp.auto_tls' => $settings['encryption'] !== 'none',
            ...
            // Send test email
            Mail::mailer('smtp')->to($request->email)->send(new TestMail);
