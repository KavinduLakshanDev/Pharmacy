...
class SystemSettingsController extends Controller
{
    ...
    public function update(Request $request)
    {
        try {
            $validated = $request->validate([
                ...
                'enable_privileged_points' => 'boolean',
            ]);
    ...
    public function updateBrand(Request $request)
    {
        try {
            $validated = $request->validate([
                ...
                'settings.themeColor' => 'nullable|string|in:blue,teal,green,purple,orange,red,custom',
    ...
    public function updateInvoiceTemplate(\Illuminate\Http\Request $request)
    {
        $request->validate([
            ...
            'template_color' => 'nullable|string|regex:/^[0-9A-Fa-f]{6}$/',
        ...
        updateSetting('invoiceTemplate', $request->template_id, $userId);

        if ($request->has('template_color')) {
            updateSetting('invoiceTemplateColor', $request->template_color, $userId);
        }

        if ($request->has('is_qr_enabled')) {
            updateSetting('isQrEnabled', $request->is_qr_enabled ? '1' : '0', $userId);
        }

        if ($request->hasFile('invoice_logo')) {
            $existingLogo = getSetting('invoiceLogo', null, $userId);
            if ($existingLogo && \Storage::disk('public')->exists($existingLogo)) {
                \Storage::disk('public')->delete($existingLogo);
            }

            $file = $request->file('invoice_logo');
            $path = $file->store('invoice-logos', 'public');
            updateSetting('invoiceLogo', $path, $userId);
        }
