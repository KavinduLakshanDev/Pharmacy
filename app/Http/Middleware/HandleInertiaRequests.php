    ...
    public function share(Request $request): array
    {
        ...
        if ($request->is('install/*') || $request->is('update/*') || ! file_exists(storage_path('installed'))) {
            ...
            try {
                ...
                try {
                    ...
                    // Fallback to default storage settings if service fails
                    $storageSettings = [
                        'allowed_file_types' => 'jpg,png,webp,gif',
                        'max_file_size_mb' => 2,
                    ];
        ...
        return [
            ...
            'flash' => [
                ...
                'last_sale' => $request->session()->get('last_sale'),
            ],
            // Provide shared enum collections for frontend forms/filters so that
            // the UI can render consistent options without hardcoding values.
            'masterTransactionEnums' => [
                'types' => MasterTransactionType::values(),
                'statuses' => MasterTransactionStatus::values(),
                'sourceTypes' => MasterTransactionSourceType::values(),
                'sourcePrefixes' => collect(MasterTransactionSourceType::cases())
                    ->mapWithKeys(fn (MasterTransactionSourceType $sourceType): array => [
                        $sourceType->value => $sourceType->referencePrefix(),
                    ])
                    ->all(),
