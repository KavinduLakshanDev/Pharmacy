...
if (! function_exists('settings')) {
    ...
    {
        // Skip database queries during installation (except in testing)
        if ((request()->is('install/*') || request()->is('update/*') || ! file_exists(storage_path('installed'))) && ! app()->environment('testing')) {
            return [];
        }
...
if (! function_exists('formatDateTime')) {
    function formatDateTime($date, $includeTime = true)
    {
        ...
        $timeFormat = $settings['timeFormat'] ?? 'h:i A';
...
if (! function_exists('currentBranch')) {
    /**
     * Active branch for inventory / POS actions (e.g. stock transfer accept).
     *
     * Resolution order: session key {@see active_branch_id}, active POS session branch,
     * then first branch belonging to the current tenant ({@see createdBy()}).
     */
    function currentBranch(): ?int
    {
        ...
        $activeSession = \App\Models\PosSession::query()
        ...
        if ($activeSession?->branch_id) {
            return (int) $activeSession->branch_id;
        }
